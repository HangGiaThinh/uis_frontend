                    ...
                    <tbody>
                        {allItems.map((item, index) => (
                            <tr key={`${item.id || item.criterionName}_${index}`} className={item.isParent ? '' : 'pl-4'}>
                                ...
                                <td>{item.banCanSuScore || ''}</td>
                                <td>{item.cvhtScore || ''}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                <div className="mt-4 flex justify-between items-center">
                    <span className="font-bold">Tổng điểm: {totalScore}</span>
                    <button type="submit" className="btn btn-primary" disabled={data.data.status !== 'WAIT_STUDENT'}>
                        Gửi đánh giá
                    </button>
                </div>
            </form>
            <Link to="/scores" className="btn btn-secondary mt-4">
                Quay lại danh sách
            </Link>
        </div>
    );
